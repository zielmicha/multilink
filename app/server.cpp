#define LOG_NAME "multilink_server"
#include "logging.h"
#include "tcp.h"
#include "tls.h"
#include "ioutil.h"
...
struct ProcessHandler {
    ...
    ProcessHandler(Reactor& reactor, std::function<void()> on_close):
        ...
        main_stream = LengthPacketStream::create(UnixSocket::connect(path));
        multilink_num = 0;
    }
    ...
    Future<int> provide_stream(StreamPtr stream) {
        auto stream = UnixSocket::connect(path);
        auto packet_stream = LengthPacketStream::create(stream);
        int stream_num = stream_counter ++;
        return send_message(packet_stream, Json::object {
                { "type", "provide-stream" },
                { "num", stream_num }
            }).then([=](unit) -> int {
                stream->set_on_read_ready(nothing);
                stream->set_on_error(nothing);
                return ioutil::write(ByteString::copy_from("OK\n"));
            }).then([=](unit) -> int {
                return stream_num;
            });
    }
    ...
    void add_link(StreamPtr stream, string name) {
        provide_stream(stream, [=](int stream_num) {
