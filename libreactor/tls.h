...
class TlsStream : public AbstractStream, public std::enable_shared_from_this<TlsStream> {
    Reactor& reactor;
    StreamPtr stream;
...
public:
    ...
    ~TlsStream();
    static std::shared_ptr<TlsStream> create(Reactor& reactor, StreamPtr stream);
...
typedef std::shared_ptr<TlsStream> TlsStreamPtr;
